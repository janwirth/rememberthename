//// EdgeQL payload for `gel` CLI — same query as the former `tuna_runtime.erl`.

import shellout

pub fn fetch_tracks_source_ids_json() -> String {
  case
    shellout.command(
      "gel",
      [
        "-I", "tuna", "-b", "main", "query", "--output-format=json",
        tracks_source_ids_edgeql,
      ],
      in: ".",
      opt: [],
    )
...
const tracks_source_ids_edgeql: String =
 ...
 cover_path := (select (
   t.integrity_checked_path_cover
 ) limit 1),
 mirror_audio_md5 := (select t.fishbone_local_track_mirror.audio.md5 limit 1),
 mirror_audio_ext := (select t.fishbone_local_track_mirror.audio.ext limit 1),
 mirror_cover_md5 := (select t.fishbone_local_track_mirror.cover.md5 limit 1),
 mirror_cover_ext := (select t.fishbone_local_track_mirror.cover.ext limit 1),
 spotify_id := (select (
